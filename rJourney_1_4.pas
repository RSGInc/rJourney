...
{user inputs}
 ...
 HouseholdFileName:string='us_synpop_hh3_sorted.dat';
 ...
 {
 WriteTripMatrix:array[0..4] of boolean=(False,False,False,False);
...
const configfname:string='rJourney_example_config.txt';
var nlogs:integer;
begin
...
type
 ...
 zoneWord = array[1..mZones] of word;
 zoneMatrix = zoneWord;
 {zoneMatrixOut = array[1..mZones] of zoneSingle;}
...
var d,oIndex,dIndex:integer; var zonesConnected, ldZonesConnected:integer;
begin
  {empty arrays}
  zonesConnected:=0;
  ldZonesConnected:=0;
...
begin
  if useOMXrail then begin
    ...
    for dindex:=1 to nZones do begin
      ...
      railBusiFare[dIndex],
      railAcEgDist[dIndex],
      railOrigStation[dindex],
      railDestStation[dindex]);
    end;
  until (oIndex=hzone);
...
begin
  ...
  if useOMXair then begin
    ...
    for dindex:=1 to nZones do begin
      ...
      airBusiFare[dIndex],
      airAcEgDist[dIndex],
      airOrigAirport[dindex],
      airDestAirport[dindex]);
    end;
  until (oIndex=hzone);
...
var xstr:string[13];
    ...
    carutil,busutil,railutil,airutil,lncdist,cdistsq,modeChoiceLogsum,destsum,carDist1way:double;
...
begin
  ...
  {for otaz:=1 to nzones do} begin
    ...
    for dtaz:=1 to nZones do begin
      {write(dtaz:5); readln;}
      carDist1way:=carDist[dtaz]/2.0;
      lncdist:=ln(max(1.0,min(carDist1way,3000)));
      cdistsq:=(min(carDist1way,3000)/100.0)*(min(carDist1way,3000)/100.0); {cap at 3000 to deal with new HI and AK destinations}

      dband:=0;
      repeat
        dband:=dband+1;
      until (carDist1way>=dbandlowlimit[dband])
        and (carDist1way< dbandhighlimit[dband]);
      ...
      for purp:=1 to nTourPurposes do begin
        ...
        if purp<5 then airfare:=airEconFare[dtaz]
                  else airfare:=airBusiFare[dtaz];
        if airfare>maxairfare[purp] then airfare:=maxairfare[purp];

        railfreqx:= railFreq[dtaz];
        if railfreqx<1 then railfreqx:=1;

        airfreqx:= airFreqDirect[dtaz]
                  +airFreq1Stop [dtaz]/2.0
                  +airFreq2Stop [dtaz]/10.0;
            ...
            for naseg:=1 to nNASegs do begin
              ...
              for moseg:=1 to nMOSegs do begin
                ...
                if modeChoiceLogsum<-900 then
                  ...
                  if (carDist[dtaz]/2.0>=50) then begin
...
(*
 if (writeProbs) then begin
   ...
   if (carDist[dtaz]/2.0>=TripMatrixMinimumDistance) then begin
...
*)
 if (writeTours) {or not(writeProbs)}  then begin
  ...
  and (carDist[dtaz]/2.0>=TripMatrixMinimumDistance) then begin
  ...
  dband:=0;
  while (dband<nTrDistBands) and (carDist[dtaz]/2.0>=trDistBandLowLimit[dband+1]) do dband:=dband+1;
  ...
  if writeTours then begin
    ...
    trAutoDistance:=carDist[dtaz];
    if trMode=1 then begin
      trTravelTime:=carTime[dtaz];
      trTravelCost:=round(carToll[dtaz]/100.0
         + trAutoDistance*copercpm);
      trOrigStation:=0;
      trDestStation:=0;
    end else
    if trMode=2 then begin
      trTravelTime:=busTime[dtaz];
      trTravelCost:=busFare[dtaz];
      trOrigStation:=0;
      trDestStation:=0;
    end else
    if trMode=3 then begin
      trTravelTime:=railTime[dtaz]
              {+ 2 *(railAcEgDist[otaz,dtaz]+railAcEgDist[dtaz,otaz])};
      if trPurpose<5
        then trTravelCost:=railEconFare[dtaz]
        else trTravelCost:=railBusiFare[dtaz];
      trOrigStation:=railOrigStation[dtaz];
      trDestStation:=railDestStation[dtaz];
    end else
    if trMode=4 then begin
      trTravelTime:=airTime[dtaz]
            + round(90/100*0*airXfer[dtaz])
            {+ 2 *(airAcEgDist[otaz,dtaz]+airAcEgDist[dtaz,otaz])};
      if trPurpose<5
        then trTravelCost:=airEconFare[dtaz]
        else trTravelCost:=airBusiFare[dtaz];
      trOrigStation:=airOrigAirport[dtaz];
      trDestStation:=airDestAirport[dtaz];
    end;
...
begin
 ...
 {load input files}
  ...
  openRoadLOSMatrices(RoadLOSFileName);
  openRailLOSMatrices(RailLOSFileName);
  openAirLOSMatrices(AirLOSFileName);

  {loop on sample households}
  writeln('Reading Households from ', HouseholdFileName, ' ....');
  ...
  repeat
    ...
    if (nHHRecs mod Sample1inX = SampleOffset) or (writeMDLogsums) then begin
      if (hhZoneIndex>lasthhZoneIndex) then begin
        readRoadLosMatrices(hhZoneIndex);
        readRailLosMatrices(hhZoneIndex);
        readAirLosMatrices(hhZoneIndex);
  ...
  until(lastHH);
  closeRoadLosMatrices;
  closeRailLosMatrices;
  closeAirLosMatrices;
